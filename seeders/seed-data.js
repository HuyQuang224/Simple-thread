...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    for (let i = 1; i <= 100; i++) {
      const randomUser = userIds[Math.floor(Math.random() * userIds.length)];
      const randomType = faker.helpers.arrayElement(["like", "comment", "follow"]);
      notifications.push({
        user_id: randomUser.id,
        type: randomType,
        reference_id: randomType === "follow" ? null : faker.number.int({ min: 1, max: 30 }), // Sửa số ngẫu nhiên
        content: `You have a new ${randomType}!`,
        read: faker.helpers.boolean(), // Sửa boolean
